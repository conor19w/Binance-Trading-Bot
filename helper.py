...
class CustomClient:
    ...
    def start_websockets(self, bots: [Bot]):
    ...
    def setup_bots(self, bots: [Bot], symbols_to_trade: [str], signal_queue, print_trades_q):
        ...
        log.info("setup_bots() - Beginning Bots setup...")
...
class Trade:
    def __init__(self, index: int, entry_price: float, position_size: float, take_profit_val: float,
                 stop_loss_val: float, trade_direction: int, order_id: int, symbol: str, coin_precision: int, tick_size: float):
        self.index = index
        self.symbol = symbol
